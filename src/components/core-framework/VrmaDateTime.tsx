...
export interface VrmaDateTimeProps {
...
const VrmaDateTime: React.FC<VrmaDateTimeProps> = ({
  mode = "datetime",
  value = null,
  onChange,
  format,
    ...
    <>
      {errorVariant === "tooltip" ? (
        <VrmaToolTip
          ...
          arrow
        >
          <Box sx={{ display: "inline-flex", width: fullWidth ? "100%" : "auto" }}>{inputBox}</Box>
        </VrmaToolTip>
      ) : (
        inputBox
      )}
