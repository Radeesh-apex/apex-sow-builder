// @/modules/sow-builder/components/Services/CmodelSelection.tsx
...
interface CommercialModelSelectionProps {
  selectedIds: Set<string>;
  onSelect: (id: string) => void;
}

export const CommercialModelSelection: React.FC<CommercialModelSelectionProps> = ({
  selectedIds = new Set(),
  onSelect,
}) => {
  ...
  return (
    <Box>
      ...
      <Grid container spacing={1.5}>
        {COMMERCIAL_MODELS_DATA.map((model) => {
                      ...
                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                        {model.deliverables.map((item, idx) => (
                          <Chip
                            ...
                            size="small"
                            variant="outlined"
                            sx={{ borderColor: 'divider', color: 'text.secondary', fontSize: '0.7rem', height: 24 }}
                          />
                        ))}
                      </Box>
                    </Box>
